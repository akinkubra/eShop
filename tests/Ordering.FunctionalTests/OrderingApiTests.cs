...
public sealed class OrderingApiTests : IClassFixture<OrderingApiFixture>
{
    ...
    [Fact]
    public async Task CompleteWithEmptyGuidFails()
    {
        var content = new StringContent(BuildOrder(), UTF8Encoding.UTF8, "application/json")
        {
            Headers = { { "x-requestid", Guid.Empty.ToString() } }
        };
        var response = await _httpClient.PutAsync("/api/v1/orders/complete", content);
    ...
    [Fact]
    public async Task CompleteNonExistentOrderFails()
    {
        var content = new StringContent(BuildOrder(), UTF8Encoding.UTF8, "application/json")
        {
            Headers = { { "x-requestid", Guid.NewGuid().ToString() } }
